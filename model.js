...
function processCow(code) {
  ...
  if (!cow || cow.type !== "cow") {
    return {
      success: false,
      message: "ไม่พบรหัสนี้ในฐานข้อมูลหรือสัตว์ไม่ใช่วัว",
    };
  }
...
function increaseUdder(code) {
  ...
  if (
    index === -1 ||
    animalsData.animals[index].type !== "cow" ||
    animalsData.animals[index].udders !== 3
  ) {
    return {
      success: false,
      message: "วัวตัวนี้ไม่สามารถเพิ่มเต้าได้ (ต้องมี 3 เต้าเท่านั้น)",
    };
  }
